...
dists = {
    ...
    "Lognormal": lognorm(s=0.5, scale=1),
    "Gumbel-1":  gumbel_r(loc=0, scale=1)
...
def task3_averaging(N=1000, m_list=[1, 2, 10, 100]):
    for name, dist in dists.items():
        ...
        for ax, m in zip(axes.flat, m_list):
            ...
            ax.set_title(f"For m={m}, theoretical mean (CLT) ~ N({theo_mu:.3f}, {theo_var:.3f}/{m} = {theo_mu:.3f})\n"
            ...
            clt_dist = stats.norm(loc=theo_mu, scale=np.sqrt(theo_var / m))
            y = clt_dist.pdf(x)
            ax.plot(x, y, 'r-', lw=2)
            # this code block ends here
